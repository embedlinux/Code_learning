...
{		
	...
	QTMR_GetDefaultConfig(&qtimerpwm_config); 				  //先设置为默认配置
	qtimerpwm_config.primarySource= qtimer_source; 		  //设置第一时钟源, 150MHz/64=2.34375MHz
	
	switch(pin)
	{
		case 30:
				QTMR_Init(TMR3,kQTMR_Channel_0,&qtimerpwm_config); 	//初始化 TIM 通道 
				QTMR_SetupPwm(TMR3,kQTMR_Channel_0, clk, duty,false,CLOCK_GetFreq(kCLOCK_IpgClk)/fredivi);
				QTMR_StartTimer(TMR3,kQTMR_Channel_0,kQTMR_PriSrcRiseEdge);
				break;
		case 33:
				QTMR_Init(TMR3,kQTMR_Channel_3,&qtimerpwm_config); 	//初始化 TIM 通道 
				QTMR_SetupPwm(TMR3,kQTMR_Channel_3, clk, duty,false,CLOCK_GetFreq(kCLOCK_IpgClk)/fredivi);
				QTMR_StartTimer(TMR3,kQTMR_Channel_3,kQTMR_PriSrcRiseEdge);
				break;
		case 40:
				QTMR_Init(TMR4,kQTMR_Channel_0,&qtimerpwm_config); 	//初始化 TIM 通道 
...
void XBARA1_Signal_Set(uint8_t inpin,uint8_t outpin)
{
  ...
  outx=output&0XFF;																		//得到真正的XBARA_OUT编号. 
	if(outx>3&&outx<20)
		IOMUXC_GPR->GPR6|=1<<(12+outx);										//GPIO做输出,则设置对应的I0为输出模式
...
{
	QTMR_PWM_Init(inpin,14,5000,0);											//初始化TIMER
	Pin_Timer_Init(outpin);															//初始化管脚输出
	
	XBARA1_Signal_Set(inpin,outpin);   									//QTMR4_CH0输出到XBARA1_INOUT17上面.
}
