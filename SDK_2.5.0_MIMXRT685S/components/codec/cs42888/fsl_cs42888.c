 ...
 * Code
 ******************************************************************************/
status_t CS42888_Init(cs42888_handle_t *handle, void *config)
{
    assert(handle != NULL);
    assert(config != NULL);
    ...
    handle->slaveAddress = CS42888_I2C_ADDR;
    ...
    if (config == NULL)
    {
        ...
        errorStatus = CS42888_SetProtocol(handle, kCS42888_BusLeftJustified, 16);
    }
    else
    {
        errorStatus = CS42888_ModifyReg(handle, CS42888_FUNCTIONAL_MODE, CS42888_FUNCTIONAL_MODE_DAC_FM_MASK,
                                        CS42888_FUNCTIONAL_MODE_DAC_FM(((cs42888_config_t *)config)->DACMode));
        errorStatus = CS42888_ModifyReg(handle, CS42888_FUNCTIONAL_MODE, CS42888_FUNCTIONAL_MODE_ADC_FM_MASK,
                                        CS42888_FUNCTIONAL_MODE_ADC_FM(((cs42888_config_t *)config)->ADCMode));
        errorStatus = CS42888_SetProtocol(handle, ((cs42888_config_t *)config)->bus, 16U);
    }
    /*Mute all DACs*/
    ...
    /*Delay and unmute*/
    errorStatus = CS42888_WriteReg(handle, CS42888_CHANNEL_MUTE, 0x0);
...
status_t CS42888_Deinit(cs42888_handle_t *handle)
{
    ...
    return CS42888_WriteReg(handle, CS42888_FUNCTIONAL_MODE, 0U);
}
