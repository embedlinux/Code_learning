...
Running the demo
================
When the demo is run, different tasks access memory with different privileges. Also secure calling
task access from the non-secure world through non-secure callable function a secure counter which
is incremented along with a non-secure counter which has to have the same value.
