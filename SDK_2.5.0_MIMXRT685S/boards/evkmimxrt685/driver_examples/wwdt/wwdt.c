...
#define APP_WDT_IRQ_HANDLER WDT0_DriverIRQHandler
...
static volatile bool timeOutResetEnable = false;
...
void APP_WDT_IRQ_HANDLER(void)
{
    ...
    if (wdtStatus & kWWDT_WarningFlag)
    {
        if (wdt_update_count < 5)
        {
            ...
            WWDT_Refresh(WWDT);
            /* If WWDT time out reset test is enabled, will refresh the WWDT 5 times
             * before going to time out reset. otherwise, will refresh WWDT every time.
             */
            if (true == timeOutResetEnable)
            {
                wdt_update_count++;
            }
        }
...
int main(void)
{
    ...
    if (WWDT_GetStatusFlags(WWDT) & kWWDT_TimeoutFlag)
    {
        ...
        /* If CPU starts from WWDT time out, disable the WWDT time out reset function. */
