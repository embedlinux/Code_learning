...
volatile uint32_t datamask = 0xFFFF7FFE;

/* Declare test functions */
int GPIO_OPEN(void);       /*  tests the CM3DS_MPS2 GPIO output enable driver functions */
int GPIO_AltFuncEN(void);  /*  tests the CM3DS_MPS2 GPIO Alternate Function driver functions */
int GPIO_IRQ(void);        /*  tests the CM3DS_MPS2 GPIO IRQ driver functions */
int GPIO_Mask_OP(void);    /*  tests the CM3DS_MPS2 GPIO Masked Accesses driver functions */
...
/* GPIO_OPEN - This function tests the CM3DS_MPS2 GPIO output enable driver functions
...
/* GPIO_AltFuncEN - This function tests the CM3DS_MPS2 GPIO Alternate Function driver functions
  GPIO Port 0 is used for this test
  - Set Alternate Function (AltFunc)
    - clear the alternate function register for all pins
    ...
    - else test passed for specified pin
    - if the AltFunc register is set to ALTMASK then the test is classed as passed
...
int GPIO_AltFuncEN(void)
{
  ...
  for(i = 1; i < 65536; i <<= 1){
    ...
    //Clear alternate function on all GPIO Port 0 pins which have an alternate function output port
...
/* GPIO_IRQ - This function tests the CM3DS_MPS2 GPIO IRQ driver functions
  ...
  - Test High level IRQ
    ...
    - for each pin on the port enable the CM3DS_MPS2 GPIO interrupt for that pin then set the Dataout on the specified pin high
  ...
  - Test Rising Edge IRQ
    ...
    - for each pin on the port enable the CM3DS_MPS2 GPIO interrupt for that pin
  ...
  - Test Low level IRQ
    ...
    - for each pin, in turn, on the port enable the CM3DS_MPS2 GPIO interrupt for that pin
...
/* GPIO_Mask_OP - This function tests the CM3DS_MPS2 GPIO Masked Accesses driver functions
