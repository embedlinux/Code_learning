 ...
 *      SVN Information
 *
 *      Checked In          : $Date: 2013-04-09 13:16:37 +0100 (Tue, 09 Apr 2013) $
 *
 *      Revision            : $Revision: 243344 $
 *
...
int main (void)
{
  ...
  if (gpio0_id_check()==0) {
    /* GPIO #0 is require for creating test input */
...
int Timer_Init(void)
{
  ...
  After CM3DS_MPS2_timer_Init_IntClock is executed,
...
int Timer_IRQ(void)
{
  ...
  - timer enabled by CM3DS_MPS2_timer_Init_IntClock, with interrupt enabled
...
int Timer_Ext(void){
  ...
  */
                               ...
                               the sampled timer values are always different*/
  {
      ...
      /* XOR output port with 0x2 so as to invert the external enable signal*/
      enabled++;
