...
#define GPIO0_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#define GPIO1_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#define GPIO2_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#define GPIO3_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#define GPIO4_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#define GPIO5_ALTERNATE_FUNC_MASK     0xFFFF    /* Implemented bits */
#include <stdio.h>
...
#define GPIO2_ALTERNATE_FUNC_DEFAULT  0x0000
#define GPIO3_ALTERNATE_FUNC_DEFAULT  0x0000
#define GPIO4_ALTERNATE_FUNC_DEFAULT  0x0000
#define GPIO5_ALTERNATE_FUNC_DEFAULT  0x0000
...
volatile int gpio2_irq_occurred;
volatile int gpio3_irq_occurred;
volatile int gpio0_irq_expected;
volatile int gpio1_irq_expected;
volatile int gpio2_irq_expected;
volatile int gpio3_irq_expected;
...
volatile int gpio1_irq_level_irq; // indicate it is a level IRQ test
                                  // Need to toggle interrupt source
volatile int gpio2_irq_level_irq; // indicate it is a level IRQ test
                                  // Need to toggle interrupt source
volatile int gpio3_irq_level_irq; // indicate it is a level IRQ test
                                  // Need to toggle interrupt source
...
int gpio_register_rw_test(CM3DS_MPS2_GPIO_TypeDef *CM3DS_MPS2_GPIO){
  ...
  if (CM3DS_MPS2_GPIO==CM3DS_MPS2_GPIO0) {
    ...
    gpio_datamask        = 0xFFFF000F;
    gpio_outen_mask        = 0xFFFFFFFF;
    gpio_altfunc_mask     = GPIO3_ALTERNATE_FUNC_MASK; /* Mask bits that are not implemented */
    gpio_altfunc_initval  = GPIO3_ALTERNATE_FUNC_DEFAULT;
    gpio_altfunc_usedmask = 0xFFFFFFFF;
  ...
  if (CM3DS_MPS2_GPIO==CM3DS_MPS2_GPIO2) {
    if (NVIC_GetPendingIRQ(PORT2_ALL_IRQn) == 0) {/* Should have been set */
      err_code = err_code | (1<<17);
      }
    }

  if (CM3DS_MPS2_GPIO==CM3DS_MPS2_GPIO3) {
    if (NVIC_GetPendingIRQ(PORT3_ALL_IRQn) == 0) {/* Should have been set */
      err_code = err_code | (1<<17);
      }
    }
