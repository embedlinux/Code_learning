// control pins:
// input: reset_n	- active low reset                                        的有效的reset信号
// input: clock		- master clock input                                      master的clock信号
// input port pins:                                                        
// input: frame_n	- must be active during whole input packet                在数据包输入期间，必须保持有效状态，低有效
...
//
// note1: frame_n must deasserted at least one cycle between packets.         在数据包与数据包之间，frame_n必须至少有一个周期是无效状态
// note2: address data does not have to wait for busy_n = 1.                  地址数据无需等待busy_n信号为1
// note3: di must wait for busy_n = 1.                                        数据必须等待busy_n信号为1
// note4: a di is taken by the chip if:  busy_n == 1 && valid_n == 0          只有当busy_n == 1 && valid_n == 1的情况下才会采样di。
// note5: frame_n must be deasserted with the last data bit in the frame.     frame_n必须在帧的最后一个数据位上变为无效状态。
// note6: once connection is successfully made, busy_n is guaranteed to       连接一旦成功的建立，busy_n必须确保在当前帧传输完成之前无效。
...
module rtslice(	reset,                                 //复位信号 ----------------------- 输入信号
				clock,                                 //时钟信号 ----------------------- 输入信号
				frame_n,                               // 1-bit 帧标志信号（低有效）----- 输入信号
				valid_n,                               // 1-bit 有效位标志信号（低有效）- 输入信号
				din,                                   // 1-bit 输入数据/地址信号 ------- 输入信号
				iarbin,                                //                                 输入信号
				arbout,                                //                                 输出信号
				arbhead,                               //                                 输入信号
				okstep,                                //                                 输出信号
				dout,                                  // 16-bit 输出信号 --------------- 输出信号
				busy_n,                                // 1-bit 忙状态输出信号 ---------- 输出信号
				valido_n,                              // 16-bit 有效位标志信号（低有效） 输出信号
				frameo_n  );                           // 16-bit 双向帧标志信号（低有效） 双向信号
